...
@php
    ...
    $defaultDescription = $setting && $setting->seo_description ? $setting->seo_description : 'This is a blog about ICT. You can get more information about ICT. I mean Web Design, Development, Computer Science, Software Development etc.';
...
@if (isset($blogPost))
    ...
    <meta name="description" content="{{ $blogPost->seo_description ? $blogPost->seo_description : $defaultDescription }}" />
    <meta name="keywords" content="{{ $blogPost->seo_keyword ? $blogPost->seo_keyword : $defaultKeyword }}" />
    <meta name="image" content="{{ $blogPost->image ? asset('assets/uploads/'.$blogPost->image) : asset($defaultImage) }}" />

    <meta property="og:title" content="{{ $blogPost->name }}" />
    <meta property="og:description" content="{{ $blogPost->seo_description ? $blogPost->seo_description : $defaultDescription }}">
    <meta property="og:keywords" content="{{ $blogPost->seo_keyword ? $blogPost->seo_keyword : $defaultKeyword }}" />
    <meta property="og:image" content="{{ $blogPost->image ? asset('assets/uploads/'.$blogPost->image) : asset($defaultImage) }}" />

@elseif (isset($blogPosts) && isset($blogCategory))
    
    {{-- Posts by Category page --}}    
    <meta name="title" content="{{ $blogCategory->name }}" />
    <meta name="description" content="{{ $blogCategory->seo_description ? $blogCategory->seo_description : $defaultDescription }}" />
    <meta name="keywords" content="{{ $blogCategory->seo_keyword ? $blogCategory->seo_keyword : $defaultKeyword }}" />
    <meta name="image" content="{{ asset($defaultImage) }}" />

    <meta property="og:title" content="{{ $blogCategory->name }}" />
    <meta property="og:description" content="{{ $blogCategory->seo_description ? $blogCategory->seo_description : $defaultDescription }}">
    <meta property="og:keywords" content="{{ $blogCategory->seo_keyword ? $blogCategory->seo_keyword : $defaultKeyword }}" />
